...
struct Tuple : Base, std::tuple<Ts...> {
    Tuple(const Ts& ... ts) : std::tuple<Ts...>{std::tuple<Ts...>(ts...)} { }
    std::string as_string() const override {
        return std::string("Tuple Node:   ")
               + ((value_type_pair(get<Ts>(*this)).first + value_type_pair(Ts{}).second + " ") + ...)
               + "\n";
    }
};

template<typename ... Ts>
struct Variant : Base, std::variant<Ts...> {
    template<typename T>
    Variant(T&& value) : std::variant<Ts...>{std::forward<T>(value)} { }
    std::string as_string() const override {
        return std::string("Variant Node: ")
               + std::visit(value_type_pair, *this).first
               + std::visit(value_type_pair, *this).second
               + "\n";
    }
};
